...
func (ec *executionContext) __resolve_entities(ctx context.Context, representations []map[string]interface{}) []fedruntime.Entity {
	...
	resolveEntity := func(ctx context.Context, typeName string, rep map[string]interface{}, idx []int, i int) (err error) {
		...
		switch typeName {
		case "MovieTrailers":
			resolverName, err := entityResolverNameForMovieTrailers(ctx, rep)
			if err != nil {
				return fmt.Errorf(`finding resolver for Entity "MovieTrailers": %w`, err)
			}
			switch resolverName {

			case "findMovieTrailersByTitle":
				id0, err := ec.unmarshalNString2string(ctx, rep["title"])
				if err != nil {
					return fmt.Errorf(`unmarshalling param 0 for findMovieTrailersByTitle(): %w`, err)
				}
				entity, err := ec.resolvers.Entity().FindMovieTrailersByTitle(ctx, id0)
				if err != nil {
					return fmt.Errorf(`resolving Entity "MovieTrailers": %w`, err)
				}
...
func entityResolverNameForMovieTrailers(ctx context.Context, rep map[string]interface{}) (string, error) {
	for {
		var (
		...
		return "findMovieTrailersByTitle", nil
	}
	return "", fmt.Errorf("%w for MovieTrailers", ErrTypeNotFound)
}
